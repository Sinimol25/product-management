$(document).ready(function(){
    console.log($("#catForm").data("action"));
    $('#catForm').validate({
    ...
    let redirect =  $('#catForm').data("url");
     $.ajax({
      url       : $("#catForm").data("action"),
      type      : "POST",
    ...
    let table;
    AjaxAdminCategoryTable();

    function AjaxAdminCategoryTable() {
        let columns = [
        ...
        if ($.fn.DataTable.isDataTable("#cat_list")) {
            $("#cat_list").DataTable().destroy();
        }

        table = $("#cat_list").DataTable({
            bProcessing: true,
            serverSide: true,
            pageLength: 10,
            paging: true,
            ...
            ajax: {
                url: $("#cat_list").data("route"), // json datasource
